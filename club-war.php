...
$stmt = $DB_con->prepare("SELECT * FROM club_war ORDER BY war_id ASC ");
$stmt->execute();
$data=$stmt->fetchAll();
?>
<!doctype html>
...
<head>
<title>Bidang MMA & Fitness Gym || Bidang Club Wars</title>
<!--meta info-->
<meta charset="utf-8">
							...
							<div class="row">
								<div class="col-md-9">
									<ul>
										...
										<li><span>//</span></li>
										<li class="category-1"><a href="#">Our Legacy</a></li>
										<li><span>//</span></li>
										<li class="category-2">Bidang Club Wars</li>								
									</ul>
								</div>	
							</div>
						</div>
											...
											<div class="product-image-wrapper">
												<div class="product-content">
													<div class="product-image product-trainer">
														<a href="club-war-details.php?war_id=<?php echo $row['war_id']; ?>"><img alt="" src="bidang-admin/images/war/<?php echo $row['photo'] ?>"></a>
													</div>
													<a href="club-war-details.php?war_id=<?php echo $row['war_id']; ?>">
														<div class="info-products">
															<div class="img-trainers">
																<div class="news-date">
																	<div class="news-day"><?php echo date("j", strtotime($row['event_date'])); ?></div>
																	<div class="news-month"><?php echo date("M", strtotime($row['event_date'])); ?></div>
																	<div class="news-month"><?php echo date("Y", strtotime($row['event_date'])); ?></div>
																</div>
															</div>
															...
															<div class="product-info">
																<div class="icon-time">
																	<span><i class="fa fa-calendar-o"></i>
																		<?php if ($row['event_start_date'] && $row['event_start_date']== 'NULL' ) { ?> YET NOT DECLARED
																	<?php } else { ?>
																		<?php if($row['event_start_date']==$row['event_end_date']) { ?>
																			<?php echo date("jS M,Y", strtotime($row['event_start_date'])); ?>
																		<?php } else { ?>
																			<?php echo date("jS M,Y", strtotime($row['event_start_date'])); ?> to <?php echo date("jS M,Y", strtotime($row['event_end_date'])); ?>
																		<?php } }?>
																	</span>
																</div>
															</div>	
